...
// Preferensi siswa juga dilambangkan dengan angka 0 dan 1, siswa yang suka sandwich berbentuk lingkaran dilambangkan dengan 0,
// siswa yang suka sandwich berbentuk persegi panjang dilambangkan dengan 1.
...
class Queue {
  ...
  requeue() {
    const savedShift = this.#lines.shift();
    this.#lines.push(savedShift);
  }
  get lines() {
    return this.#lines;
  }
...
function countStudentsCantEat(students, sandwiches) {
  ...
  let isStopped = false;
  let counter = studentQueue.lines.length;
  while (!isStopped) {
    if (counter > 0) {
      if (studentQueue.head === sandwichQueue.head) {
        studentQueue.dequeue();
        sandwichQueue.dequeue();
        counter = studentQueue.lines.length;
      } else {
        studentQueue.requeue();
        counter -= 1;
      }
    } else {
      isStopped = true;
  ...
  return studentQueue.lines.length;
}
